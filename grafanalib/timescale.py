"""Helpers for Timescale-driven graphs."""
...
def PromGraph(data_source, title, expressions, **kwargs):
    ...
    if all(isinstance(rawSql, dict) for rawSql in expressions):
        targets = [
            G.Target(refId=refId, **args)
            for (args, refId) in zip(expressions, letters)]
    else:
        targets = [
            G.Target(rawSql=rawSql, rawQuery=rawQuery, legendFormat=legendFormat, refId=refId)
            for ((rawQuery, rawSql, legendFormat), refId) in zip(expressions, letters)]
    return G.Graph(
        title=title,
